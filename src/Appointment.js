...
function Appointment(props) {
    ...
    const sendSMS = () => {
        async function doSMS() {
            let payload={id:props.id,msg:document.getElementById("msg").value,tel:document.getElementById("tel").value, date: document.getElementById("date").value, time: document.getElementById("time").value}
            console.log (payload)
...
//   const [isOpen, setIsFormOpen] = useState(false);
//   const [isBooked, setIsBooked] = useState(false);
//   const [sendHouse, setSend] = useState(null);

//   useEffect(() => {
//     if (sendHouse) {
//         async function doSMS() {
//           let payload = { msg: document.getElementById(`msg-${sendHouse}`).value, tel: document.getElementById(`tel-${sendHouse}`).value };
//           console.log(payload);
//           let response = await fetch("http://localhost:3001/sms", {
//             method: "POST",
